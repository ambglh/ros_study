...
typedef actionlib::SimpleActionServer<ghm_action::ghm_AddintAction> Server;


void cb(const ghm_action::ghm_AddintGoalConstPtr &goal,Server* server){
    //获取目标值
    int num = goal->num;
    ROS_INFO("目标值:%d",num);
    //累加并响应连续反馈
    int result = 0;
    ghm_action::ghm_AddintFeedback feedback;//连续反馈
